const SPOTIFY_HTML = `<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Yuki Store - Spotify Finder</title>
    <meta property="og:title" content="Yuki Store - Spotify Finder">
...
<body class="font-inter">
    ...
    <aside id="sidebar" class="overflow-x-hidden overflow-y-hidden max-w-full">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="?page=spotify" class="nav-item active">
        ...
        <div class="page-container">
            <div class="section-header">
                <div class="section-icon bg-emerald-50">
                    <i data-lucide="music" class="w-5 h-5 text-emerald-500"></i>
                </div>
                <div>
                    <h2 class="text-xl font-bold text-[var(--text-primary)]">Spotify Finder</h2>
                    <p class="text-sm text-[var(--text-muted)]">Cari lagu, lihat lirik, dan dengarkan preview</p>
                </div>
            </div>

            <!-- DOWNLOAD BY URL SECTION -->
            <div class="glass-card p-6 mb-6" style="border-color: rgba(16,185,129,0.2);">
                <div class="flex items-center gap-2 mb-3">
                    <i data-lucide="download" class="w-4 h-4 text-emerald-500"></i>
                    <h3 class="font-bold text-[var(--text-primary)] text-sm">Spotify Audio Downloader</h3>
                </div>
                <form id="spotify-dl-form" class="flex gap-3">
                    <div class="search-box flex-1">
                        <i data-lucide="link" class="search-icon w-4 h-4"></i>
                        <input type="text" id="spotify-url" placeholder="Paste URL Spotify di sini..." required>
                    </div>
                    <button type="submit" class="btn-primary btn-emerald px-5 whitespace-nowrap"><i data-lucide="download" class="w-4 h-4"></i><span class="hidden sm:inline">Unduh</span></button>
                </form>
                <p class="text-[11px] text-[var(--text-muted)] mt-2">* Buka aplikasi Spotify → Share → Copy Link, lalu paste di atas.</p>
            </div>

            <!-- SEARCH SECTION -->
            <div class="glass-card p-6 mb-10">
                <form id="spotify-form" class="flex gap-3 mb-6">
                    <div class="search-box flex-1">
                        <i data-lucide="search" class="search-icon w-4 h-4"></i>
                        <input type="text" id="spotify-query" placeholder="Cari lagu, artis, atau album..." required>
                    </div>
                    <button type="submit" class="btn-primary btn-emerald px-6"><i data-lucide="search" class="w-4 h-4"></i><span class="hidden sm:inline">Cari</span></button>
                </form>
                <div id="spotify-loading" class="hidden">
                    <div class="flex flex-col items-center justify-center py-12">
                        <div class="loading-spinner mb-3"></div>
                        <p class="text-sm text-[var(--text-muted)]">Mencari lagu...</p>
                    </div>
                </div>
                <div id="spotify-results" class="space-y-3"></div>
                <div id="spotify-empty" class="text-center py-12">
                    <div class="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center mx-auto mb-3">
                        <i data-lucide="music-2" class="w-8 h-8 text-emerald-300"></i>
                    </div>
                    <p class="text-sm text-[var(--text-muted)]">Masukkan judul lagu untuk mulai mencari</p>
                </div>
            </div>
...
if (typeof YUKI !== 'undefined' && YUKI.registerPage) {
    YUKI.registerPage('spotify', SPOTIFY_HTML);
}

console.log('✅ YUKI STORE - Spotify page registered');